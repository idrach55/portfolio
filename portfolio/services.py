...
class AlphaVantage:
    def __init__(self, api_key: Optional[str] = None):
        ...
        if self.api_key is None:
            self.api_key = (
                open(f"{KEY_DIR}/alphavantage.keys", "r").read().split("\n")[0]
            )
    ...
    def chart(self, symbol: str, adjusted: bool = True) -> pd.DataFrame:
        ...
        url = self.root_url + "function={}&symbol={}&outputsize=full&apikey={}".format(
        ...
        assert (
            r.json().get("Time Series (Daily)") is not None
    ...
    def earnings(self, symbol: str):
        url = self.root_url + "function=EARNINGS&symbol={}&apikey={}".format(
            symbol, self.api_key
        )
        r = requests.get(url)
        data = r.json()
    ...
    def quote(self, symbol: str) -> pd.DataFrame:
        url = self.root_url + "function=GLOBAL_QUOTE&symbol={}&apikey={}".format(
            symbol, self.api_key
        )
        r = requests.get(url)
        return r.json()
    ...
    def overview(self, symbol: str) -> pd.DataFrame:
        url = self.root_url + "function=OVERVIEW&symbol={}&apikey={}".format(
            symbol, self.api_key
        )
        r = requests.get(url)
        return r.json()
