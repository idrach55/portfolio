...
class Garch:
    ...
    def getLongRunVol(fit, scale=100):
        return (
            np.sqrt(
                fit.params["omega"]
                / (1.0 - fit.params["alpha[1]"] - fit.params["beta[1]"])
                * 252
            )
            / scale
        )
    ...
    def fitMulti(symbols):
        ...
        for symbol in symbols:
            ...
            epsilon[symbol] = (
                100.0 * returns[symbol] / fits[symbol].conditional_volatility
            )
        ...
        for idx_i in range(len(symbols)):
            for idx_j in range(len(symbols)):
                corr[idx_i][idx_j] = epsilon[symbols[idx_i]][shared_idx].corr(
                    epsilon[symbols[idx_j]][shared_idx]
                )
    ...
    def getMCPaths(
        ...
        noise = np.random.multivariate_normal(
            [0.0] * len(drifts), corr, size=(num_paths, num_steps)
        )
        for symbol_idx in range(len(drifts)):
            for t in range(num_steps):
                if t > 0:
                    sigma_2[:, t, symbol_idx] = (
                        params.omega[symbol_idx]
                        + params.alpha[symbol_idx] * paths[:, t - 1, symbol_idx] ** 2
                        + params.beta[symbol_idx] * sigma_2[:, t - 1, symbol_idx]
                    )
                paths[:, t, symbol_idx] = (
                    np.sqrt(sigma_2[:, t, symbol_idx]) * noise[:, t, symbol_idx]
                )
...
class MCPortfolio:
    ...
    def genPaths(self, years=10, drifts=None, vols=None, N=1000):
        ...
        self.paths = np.concatenate(
            [np.ones(shape=(paths.shape[0], 1, paths.shape[2])), paths], axis=1
        )

    def build(
        self,
        init_value: float,
        withdraw_fixed: Union[float, pd.Series, List[float]],
        withdraw_pct: Union[float, List[float]],
        fee=0.00,
        withdraw_explicit=None,
    ):
        N = self.paths.shape[0]
        qtr_index = np.arange(0, (self.paths.shape[1] - 1) // 63 + 1) * 63
        self.shares = np.zeros(shape=(N, qtr_index.shape[0], self.paths.shape[2])) + (
            self.basket.values * init_value
        )
        ...
        for qtr in range(1, len(qtr_index)):
            ...
            self.value[:, qtr] = np.sum(
                self.shares[:, qtr - 1] * self.paths[:, qtr_index[qtr]], axis=1
            )
            self.income[:, qtr] = (
                np.sum((self.basket * self.yields / 4) * (1.0 - self.taxes))
                * self.value[:, qtr - 1]
            )
            self.taken[:, qtr] = (
                withdraw_fixed[(qtr - 1) // 4] / 4
                + self.value[:, qtr - 1] * withdraw_pct[(qtr - 1) // 4] / 4
            )
            ...
            self.resid[:, qtr] = (
                self.income[:, qtr]
                - self.value[:, qtr - 1] * fee / 4
                - self.taken[:, qtr]
            )
            self.value[:, qtr] += self.resid[:, qtr]
            self.shares[:, qtr] = (
                np.outer(self.value[:, qtr], self.basket.values)
                / self.paths[:, qtr_index[qtr]]
            )

            # Rebalance and charge capital gains taxes (assuming all long-term.)
            shares_to_trade = self.shares[:, qtr] - self.shares[:, qtr - 1]
            amended_basis = (
                self.costs[:, qtr - 1] * self.shares[:, qtr - 1]
                + shares_to_trade * self.paths[:, qtr_index[qtr]]
            ) / np.maximum(self.shares[:, qtr], 1.0)
            self.costs[:, qtr] = (shares_to_trade > 0) * amended_basis + (
                shares_to_trade < 0
            ) * self.costs[:, qtr - 1]
            ...
            self.gains[:, qtr] = (
                (shares_to_trade < 0)
                * -shares_to_trade
                * (self.paths[:, qtr_index[qtr]] - self.costs[:, qtr - 1])
            ).sum(axis=1)
            self.value[:, qtr] -= (
                np.maximum(self.gains[:, qtr], 0.0) * self.brackets.getLTCapGains()
            )

        # Snap negative portfolios to zero.
        for path_idx, step_idx in zip(
            np.where(self.value <= 0)[0], np.where(self.value <= 0)[1]
        ):
